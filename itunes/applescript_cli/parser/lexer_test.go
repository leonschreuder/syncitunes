...
func Test__should_lex_songs(t *testing.T) {
	...
	assert.Equal(t, item{filePathItem, "Macintosh HD"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "Users"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "someuser"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "music"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "playlist"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "Some Artist - Some Song.mp3"}, <-l.items)
	assert.Equal(t, item{closeQuote, "\""}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
...
func Test__should_lex_multiple_songs(t *testing.T) {
	l := lex(`{{"playlist", 1, -1, {alias "Macintosh HD:Users:someuser:music:playlist:Some Artist - Some Song.mp3", alias "Macintosh HD:Users:someuser:music:playlist:Some Artist - Some Other Song.mp3"}}}`)
	...
	assert.Equal(t, item{filePathItem, "playlist"}, <-l.items)
	assert.Equal(t, item{filePathSeparator, ":"}, <-l.items)
	assert.Equal(t, item{filePathItem, "Some Artist - Some Other Song.mp3"}, <-l.items)
	assert.Equal(t, item{closeQuote, "\""}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
...
func Test__should_lex_multiple_items_with_songs(t *testing.T) {
	l := lex(`{{"root", 1, -1, {}},{"playlist", 2, 1, {alias "someSong.mp3"}}}`)

	assert.Equal(t, item{opener, "{"}, <-l.items)
	assert.Equal(t, item{opener, "{"}, <-l.items)
	assert.Equal(t, item{openQuote, "\""}, <-l.items)
	assert.Equal(t, item{itemName, "root"}, <-l.items)
	assert.Equal(t, item{closeQuote, "\""}, <-l.items)
	assert.Equal(t, item{separator, ","}, <-l.items)
	assert.Equal(t, item{itemId, "1"}, <-l.items)
	assert.Equal(t, item{separator, ","}, <-l.items)
	...
	assert.Equal(t, item{closeQuote, "\""}, <-l.items)
	assert.Equal(t, item{separator, ","}, <-l.items)
	assert.Equal(t, item{itemId, "2"}, <-l.items)
	assert.Equal(t, item{separator, ","}, <-l.items)
	assert.Equal(t, item{itemId, "1"}, <-l.items)
	assert.Equal(t, item{separator, ","}, <-l.items)
	assert.Equal(t, item{opener, "{"}, <-l.items)
	assert.Equal(t, item{aliasIndicator, "alias"}, <-l.items)
	assert.Equal(t, item{filePathOpener, "\""}, <-l.items)
	assert.Equal(t, item{filePathItem, "someSong.mp3"}, <-l.items)
	assert.Equal(t, item{closeQuote, "\""}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
	assert.Equal(t, item{closer, "}"}, <-l.items)
